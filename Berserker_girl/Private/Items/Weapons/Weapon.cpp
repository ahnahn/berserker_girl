...
void AWeapon::OnBoxOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (LastDamageTimes.Contains(OtherActor)) {
		...
		if (CurrentTime - LastDamageTime < 0.3f) {  // Change 1.0f to the desired time interval
			return;
		}
	}

	DamagedActors.Add(OtherActor);
	LastDamageTimes.Add(OtherActor, GetWorld()->GetTimeSeconds());  // Update the last damage time
...
void AWeapon::BoxTrace(FHitResult& BoxHit)
{
	...
	if (BoxHit.GetActor()) UE_LOG(LogTemp, Warning, TEXT("Hit Actor: %s"), *BoxHit.GetActor()->GetName());

	if (BoxHit.GetComponent()) UE_LOG(LogTemp, Warning, TEXT("Hit Component: %s"), *BoxHit.GetComponent()->GetName());
}